    ...
    .then(function (data) {
      ...
      data.forEach((pokemon) => {
        if (data.indexOf(pokemon) <= 167) {
          ...
          data.indexOf(pokemon) >= 168 &&
          data.indexOf(pokemon) <= 271
        ) {
          season2Pokemons.push(pokemon);
        } else if (
          data.indexOf(pokemon) >= 271 &&
          data.indexOf(pokemon) <= 431
        ) {
          season3Pokemons.push(pokemon);
        } else if (
          data.indexOf(pokemon) >= 432 &&
          data.indexOf(pokemon) <= 552
        ) {
...
function drawResumeDatas(orderedDatas, currentSeason) {
  ...
  resumeSvg
    .attr("width", resumeWidth + margin.left + margin.right)
  ...
  reumeButtons.forEach((button) => {
    button.addEventListener("click", (e) => {
      let currentSeason = e.target.dataset.season;
...
function drawForcesFaiblessesData(fetchedData, elementToDisplay) {
  ...
  let groupeForcesFaiblesses = forcesFaiblessesSvg
    .append("g")
    .attr("class", "forces-faiblesses-drawn");

  //Faiblesses
  let selectedType = elements.filter((el) => el.Type1 == elementToDisplay);
  // console.log("selectedType", selectedType[0].Type1)
  groupeForcesFaiblesses
    .selectAll("forcesEtFaiblessesList")
    .data(selectedType[0].Type1)
    .join((enter) =>
      enter
        .append("text")
        .attr("class", "faiblessesHeader")
        .attr("width", "400")
        .attr("y", 100)
        .attr("x", 54)
        .attr("font-size", 24)
  ...
  listeElementsSvg
    .attr("width", ElementsListWidth + margin.left + margin.right)
    .attr("height", ElementsListHeight + margin.top + margin.bottom)
    .attr("transform", "translate(" + margin.left + "," + margin.top + ")");
  let groupeElementsList = listeElementsSvg
    .append("g")
    .attr("class", "elements-list-drawn");
  ...
  elementsGroups.forEach((svgGroup) => {
    ...
    svgGroup.addEventListener("mouseleave", (e) => {
...
function drawPokemonsParType(fetchedData) {
  ...
  //Dimensions du svgs montrant les forces et les faiblesses
  const margin = { top: 10, right: 40, bottom: 10, left: 40 };
  const nbParTypeWidth =
    screen.width - margin.left - margin.right - screen.width * 0.15;
  ...
  nbParTypeSvg
    .attr("width", nbParTypeWidth + margin.left + margin.right)
    .attr("height", nbParTypeHeight + margin.top + margin.bottom)
    .attr("transform", "translate(" + margin.left + "," + margin.top + ")");
  let groupeNbParType = nbParTypeSvg
    .append("g")
    .attr("class", "np-par-type-drawn");
  let groupNbParTypeText = nbParTypeSvg
    .append("g")
    .attr("class", "np-par-type-text-drawn");
  // console.log("nbPerElements", nbPerElements)

  const y = d3.scaleLinear().domain([0, 112]).range([nbParTypeHeight, 0]);
...
function drawSchemaElements() {
  ...
  //Dessin du schema selon l'attribut sélectionné:
  console.log(htmlList.children);
  document
    .querySelector(".dynamic-schema")
    .setAttribute("href", "./img/types-svg/All.svg");
  htmlList.addEventListener("click", (e) => {
    ...
    document.querySelector(
      ".schema-element-name"
    ).textContent = `${selectedElement} element`;
  });
}
